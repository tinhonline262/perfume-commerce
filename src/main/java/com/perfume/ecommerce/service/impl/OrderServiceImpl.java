...
public class OrderServiceImpl implements OrderService {
    ...
    public Order postOrder(Order order, Map<Long, Long> perfumesId) {
        ...
        if (com.perfume.ecommerce.enums.PaymentMethod.COD.equals(order.getPaymentMethod())) {
            String subject = "Order #" + order.getId();
            String template = "order-template";
            Map<String, Object> attributes = new HashMap<>();
            attributes.put("order", order);
            mailSender.sendMessageHtml(order.getEmail(), subject, template, attributes);
            order.setPaymentStatus(com.perfume.ecommerce.enums.PaymentStatus.SUCCESS);
            orderRepository.save(order);
        }
